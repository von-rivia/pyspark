...
SELECT *,
    array_join(dynamic_case_statements, ', ') AS dynamic_query
FROM dynamic_query;


-- Creación tabla resultados evolutivo 
CREATE TABLE sandbox.ganafinanciero.resultados_evolutivo_diario
USING DELTA 
AS (
    WITH fechas_unicas AS (
        SELECT DISTINCT fecha 
